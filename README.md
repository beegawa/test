...
1. **파이썬 설치** — [python.org](https://www.python.org/downloads/) 에서 받아 설치.
   설치 화면에서 **"Add Python to PATH" 를 반드시 체크**하세요.
   (또는 명령 프롬프트에서 `winget install Python.Python.3.12`)
2. 이 저장소를 내려받아 압축을 풉니다.
3. PowerShell 을 열고 그 폴더에서:
   ```powershell
   .\scripts\setup-windows.ps1
   ```
   ffmpeg 설치 · 파이썬 패키지 설치 · 준비 상태 점검까지 한 번에 합니다.
   실행이 막히면 `powershell -ExecutionPolicy Bypass -File .\scripts\setup-windows.ps1`

### 실행

**`예약녹화-시작.bat` 을 더블클릭**하면 브라우저가 열립니다.
...
`python -m webrec doctor` 를 실행하면 소리 장치를 찾았는지 알려줍니다.
못 찾으면 **영상만 녹화되고**, 그 사실이 점검 결과와 메일에 표시됩니다.

**YouTube · Twitch 등 스트림 녹화는 위 제약이 없습니다.** 화면을 찍는 게 아니라
영상 데이터를 직접 받아 저장하므로, 화면이 꺼져 있어도 되고 다른 작업을 해도 됩니다.

> Windows 용 캡처(gdigrab/dshow)는 제가 Windows 장비에서 직접 실행해 보지 못했습니다.
> 명령 구성과 장치 선택 로직은 테스트로 검증했지만, 실제 녹화는 예약 전에
> `python -m webrec doctor` 와 화면의 `지금 10초 점검` 으로 꼭 확인해 주세요.
