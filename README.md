...
| **재생 상태** | 재생 버튼을 못 눌러 **정지 화면**이 그대로 녹화되는 경우 |
| **소리 장치** | 소리를 켰는데 녹음할 장치가 없어 무음으로 녹화되는 경우 |
...
못 찾으면 **영상만 녹화되고**, 10초 사전 점검이 `소리 장치` 항목을 실패로 알려줍니다
(기본 설정에서는 그래도 녹화는 진행됩니다).
...
화면 전체가 아니라 **영상이 나오는 부분만** 녹화합니다. 웹 화면의 `녹화 영역` 에서 고를 수 있습니다.

| 설정 | 동작 |
|---|---|
| **영상 부분만** (기본) | 영상을 전체화면으로 만들어 보고, 안 되면 영상 요소 위치만큼 잘라서 녹화 |
| 화면 전체 | 예전처럼 화면 전체 (다른 창·알림도 같이 찍힘) |
| 직접 지정 | `x,y,너비,높이` 로 좌표 입력 |

영상 위치는 **계산이 아니라 측정**합니다. 페이지에 잠깐 표식을 띄우고 화면을 한 장 찍어
그 표식이 실제로 찍힌 자리를 읽습니다. 그래서 창 위치·화면 배율(125% 등)·전체화면 여부와
상관없이 정확합니다.

> **YouTube 라이브는 `캡처 방식: 스트림` 이 가장 좋습니다.** 화면을 찍는 게 아니라 영상을
> 직접 받아 저장하므로 원본 화질 그대로이고, 화면이 꺼져 있어도 됩니다. 영역 문제 자체가 없습니다.

### 자동 재생

브라우저에서 영상이 **정지 상태로 녹화되는 사고**를 막기 위해, 재생 버튼 클릭 → `play()` 호출 →
키보드 단축키 순으로 시도하고 **재생 위치가 실제로 흐르는지**까지 확인합니다.
끝내 재생되지 않으면 사전 점검이 실패로 알려줍니다.
...
내 PC 가 꺼져 있으면 녹화도 안 됩니다. 항상 켜져 있는 서버로 옮기려면
**[deploy/README.md](deploy/README.md)** 를 보세요. 무료 VM·서버리스·컨테이너 방법과
각각의 실제 한도·비용, 주의할 점을 정리해 두었습니다.
